...
import { calculateLineLengthFromString } from "@/features/read-book/utils/calculate-line-length-from-string.ts";

export const splitContentIntoALine = (contents: ContentData[]): ContentData[] => {
  const splited: ContentData[] = contents.map(({ isNeedIndent, type, content }, index) => {
    ...
    if(content.length >= 36) {
      ...
      if(remindedSplitedString.length >= 2) {
        ...
        tmpData.push({
          ...
          isNeedBlankLine: type === "p" && contents[index+1] && contents[index+1].type === "h1" ? true : false,
          type: type,
    ...
    return {
      isNeedIndent: isAngleBrackets ? false : isNeedIndent,
      isNeedLineBreak: true,
      isNeedBlankLine: type === "h1" || type === "p" && contents[index+1].type === "h1",
      type: type,
      content: content,
    }
  })
